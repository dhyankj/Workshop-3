...
/** Filters supported by the game listing helper. */
...
/** All categories ordered by name for listing filter controls. */
export async function getAllCategories(db: Database): Promise<Category[]> {
    return db.select({ id: categories.id, name: categories.name }).from(categories).orderBy(asc(categories.name));
}

/** All publishers ordered by name for listing filter controls. */
